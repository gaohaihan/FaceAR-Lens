...
(function (SceneUtils) {
    ...
    function delay(func, script, time) {
        const dl = script.createEvent("DelayedCallbackEvent");
        dl.bind(func);
        dl.reset(time);
    }
    SceneUtils.delay = delay;
    function execOnNextFrame(func, script) {
        delay(func, script, 0);
    }
    SceneUtils.execOnNextFrame = execOnNextFrame;
